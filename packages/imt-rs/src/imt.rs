        ...
        for level in 0..depth {
            for index in 0..((imt.nodes[level].len() as f64 / arity as f64).ceil() as usize) {
                ...
                let children: Vec<_> = (0..arity)
                    .map(|i| imt.nodes[level].get(position + i).cloned().unwrap_or_else(|| imt.zeroes[level].clone()))
                    .collect();
    ...
    pub fn update(&mut self, mut index: usize, new_leaf: IMTNode) -> Result<(), &'static str> {
        ...
        for level in 0..self.depth {
            ...
            let children: Vec<_> = (level_start_index..level_end_index)
                .map(|i| self.nodes[level].get(i).cloned().unwrap_or_else(|| self.zeroes[level].clone()))
                .collect();
    ...
    pub fn create_proof(&self, index: usize) -> Result<IMTMerkleProof, &'static str> {
        ...
        for level in 0..self.depth {
            ...
            for i in level_start_index..level_end_index {
                if i != current_index {
                    level_siblings.push(self.nodes[level].get(i).cloned().unwrap_or_else(|| self.zeroes[level].clone()));
                }
            }
...
mod tests {
    ...
    fn should_not_insert_in_full_tree() {
        let hash: IMTHashFunction = simple_hash_function;
        let mut imt = IMT::new(hash, 1, "zero".to_string(), 2, vec!["leaf1".to_string(), "leaf2".to_string()]).unwrap();
    ...
    fn test_root() {
        let hash: IMTHashFunction = simple_hash_function;
        let mut imt = IMT::new(hash, 2, "zero".to_string(), 2, vec!["leaf1".to_string(), "leaf2".to_string()]).unwrap();
    ...
    fn test_leaves() {
        let hash: IMTHashFunction = simple_hash_function;
        let imt = IMT::new(hash, 2, "zero".to_string(), 2, vec!["leaf1".to_string(), "leaf2".to_string()]).unwrap();
