...
/**
 * Calculate the poseidon hash of 4 inputs.
 ...
 */
export default function poseidon4(inputs: bigint[], cacheConstants = false) {
    if (cacheConstants && !cachedConstants) {
        cachedConstants = getConstants(C, M)
    }

    return poseidonBasic(4, cachedConstants ?? getConstants(C, M))([0n, ...inputs])[0]
}
