...
/**
 * Calculate the poseidon hash of 5 inputs.
 ...
 */
export default function poseidon5(inputs: bigint[], cacheConstants = false) {
    if (cacheConstants && !cachedConstants) {
        cachedConstants = getConstants(C, M)
    }

    return poseidonBasic(5, cachedConstants ?? getConstants(C, M))([0n, ...inputs])[0]
}
