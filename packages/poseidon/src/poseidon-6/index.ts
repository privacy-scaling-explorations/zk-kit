...
/**
 * Calculate the poseidon hash of 6 inputs.
 ...
 */
export default function poseidon6(inputs: bigint[], cacheConstants = false) {
    if (cacheConstants && !cachedConstants) {
        cachedConstants = getConstants(C, M)
    }

    return poseidonBasic(6, cachedConstants ?? getConstants(C, M))([0n, ...inputs])[0]
}
