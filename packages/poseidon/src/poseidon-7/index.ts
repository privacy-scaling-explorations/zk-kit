...
/**
 * Calculate the poseidon hash of 7 inputs.
 ...
 */
export default function poseidon7(inputs: bigint[], cacheConstants = false) {
    if (cacheConstants && !cachedConstants) {
        cachedConstants = getConstants(C, M)
    }

    return poseidonBasic(7, cachedConstants ?? getConstants(C, M))([0n, ...inputs])[0]
}
