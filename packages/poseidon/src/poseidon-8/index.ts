...
/**
 * Calculate the poseidon hash of 8 inputs.
 ...
 */
export default function poseidon8(inputs: bigint[], cacheConstants = false) {
    if (cacheConstants && !cachedConstants) {
        cachedConstants = getConstants(C, M)
    }

    return poseidonBasic(8, cachedConstants ?? getConstants(C, M))([0n, ...inputs])[0]
}
